...
{  
 ...
 {   
   ...
   s2.erase(pos,pos+1);
    A[k]=s2;
    cout<<"Query AND: "<<and_query_type<<endl;
    cout<<"Query OR: "<<or_query_type<<endl;
    for(int i=1;i<=k;i++)
    {
    cout<<A[i]<<i<<endl;
    }
...
{
   int m=0,n=1;
    while(n<=or_operation)
    {
      R[1]=add(R[1],R[1+and_operation+n]);   // To merge multiple posting list for OR operation
      n++;
    }
